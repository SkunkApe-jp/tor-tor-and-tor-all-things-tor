...
class MasterNetworkAnalyzer:
    ...
    def find_screenshot_path(self, full_url):
        ...
        if path_parts:
            ...
            if os.path.exists(expected_path):
                ...
                return rel_path.replace('\\', '/'), self.get_image_dimensions(expected_path)
        else:
            index_path = os.path.join(image_dir, "index.png")
            if os.path.exists(index_path):
                rel_path = os.path.relpath(index_path, self.scraped_data_dir)
                return rel_path.replace('\\', '/'), self.get_image_dimensions(index_path)
            ...
            if os.path.exists(onion_path):
                ...
                return rel_path.replace('\\', '/'), self.get_image_dimensions(onion_path)
        ...
        for onion_addr in self.onion_sites:
            ...
            for link in outbound_links:
                ...
                if onion_match:
                    ...
                    link_html_file = os.path.join(self.scraped_data_dir, linked_onion, 'htmls', f"{linked_onion}.html")
    ...
    def detect_communities(self):
        ...
        elapsed = time.time() - start_time
        print(f"Community detection completed in {elapsed:.2f} seconds, found {len(self.communities)} communities")
    ...
    def generate_all_visualizations(self):
        ...
        # 2. Community Heatmap
        viz_files['heatmap'] = generate_community_heatmap(self.G, self.communities, self.scraped_data_dir)
        
        # 3. Anomaly Detection
        viz_files['anomalies'] = detect_anomalies(self.G, self.communities, self.scraped_data_dir)
        ...
        viz_files['advanced'] = generate_all_advanced_visualizations(self.G, self.communities, self.scraped_data_dir)
    ...
    def generate_standard_network(self):
        ...
        plt.title(f"Grand Onion Network ({self.G.number_of_nodes()} nodes, {len(self.communities)} communities)", 
                 fontsize=24)
    ...
    def generate_circular_layout(self):
        ...
        plt.title(f"Circular Network Layout ({self.G.number_of_nodes()} nodes)", fontsize=20)
    ...
    def generate_hierarchical_layout(self):
        ...
        plt.title(f"Hierarchical Community Layout ({len(self.communities)} communities)", fontsize=20)
    ...
    def generate_centrality_visualizations(self):
        ...
        axes[0,0].hist(degree_values, bins=50, alpha=0.7, color='blue', edgecolor='black')
        ...
        axes[0,1].hist(between_values, bins=50, alpha=0.7, color='red', edgecolor='black')
        ...
        axes[1,0].hist(close_values, bins=50, alpha=0.7, color='green', edgecolor='black')
        ...
        axes[1,1].hist(eigen_values, bins=50, alpha=0.7, color='purple', edgecolor='black')
    ...
    def generate_degree_analysis(self):
        ...
        degree_clustering_pairs = [(self.G.degree(n), clustering[n]) for n in self.G.nodes()]
        ...
        axes[1,1].set_xticklabels([f"#{i+1}" for i in range(len(node_names))], rotation=45)
    ...
    def generate_temporal_analysis(self):
        ...
        axes[0,0].text(0.5, 0.5, 'Temporal Analysis\n(Timestamp data not available)', 
                       ha='center', va='center', transform=axes[0,0].transAxes, fontsize=16)
        axes[0,0].set_title('Network Growth Over Time')
        
        axes[0,1].text(0.5, 0.5, 'Community Evolution\n(Historical data needed)', 
                       ha='center', va='center', transform=axes[0,1].transAxes, fontsize=16)
        axes[0,1].set_title('Community Evolution')
        
        axes[1,0].text(0.5, 0.5, 'Node Lifecycle\n(Timestamp data required)', 
                       ha='center', va='center', transform=axes[1,0].transAxes, fontsize=16)
        axes[1,0].set_title('Node Lifecycle Analysis')
        
        axes[1,1].text(0.5, 0.5, 'Connection Patterns\n(Time series data needed)', 
    ...
    def generate_subgraph_analysis(self):
        ...
        for idx, (comm_id, size) in enumerate(top_communities):
            ...
            # Draw subgraph
            axes[idx].axis('off')
            nx.draw_networkx_edges(subgraph, pos, ax=axes[idx], alpha=0.3, edge_color='gray', width=0.5)
            nx.draw_networkx_nodes(subgraph, pos, ax=axes[idx], node_size=node_sizes, 
    ...
    def generate_network_dashboard(self):
        ...
        # Title
        fig.suptitle('Master Network Analysis Dashboard', fontsize=20, fontweight='bold')
        ...
        centrality_means = [
            np.mean(list(self.analysis_results['metrics']['degree_centrality'].values())),
            np.mean(list(self.analysis_results['metrics']['betweenness_centrality'].values())),
            np.mean(list(self.analysis_results['metrics']['closeness_centrality'].values())),
            np.mean(list(self.analysis_results['metrics']['eigenvector_centrality'].values()))
        ]
        bars = ax4.bar(centrality_types, centrality_means, color=['red', 'blue', 'green', 'orange'])
        ...
        clustering_values = list(self.analysis_results['metrics']['clustering'].values())
        ...
        if self.analysis_results['metrics']['is_connected']:
            ax6.text(0.5, 0.7, 'Network is CONNECTED', ha='center', va='center', 
                    transform=ax6.transAxes, fontsize=14, fontweight='bold', color='green')
            ax6.text(0.5, 0.5, f"Diameter: {self.analysis_results['metrics']['diameter']}", 
                    ha='center', va='center', transform=ax6.transAxes, fontsize=12)
            ax6.text(0.5, 0.3, f"Avg Path Length: {self.analysis_results['metrics']['avg_shortest_path']:.2f}", 
                    ha='center', va='center', transform=ax6.transAxes, fontsize=12)
        else:
            ax6.text(0.5, 0.7, 'Network is DISCONNECTED', ha='center', va='center', 
                    transform=ax6.transAxes, fontsize=14, fontweight='bold', color='red')
            ax6.text(0.5, 0.5, f"Components: {self.analysis_results['metrics']['components']}", 
                    ha='center', va='center', transform=ax6.transAxes, fontsize=12)
            ax6.text(0.5, 0.3, f"Largest Component: {self.analysis_results['metrics']['largest_component_size']} nodes", 
                    ha='center', va='center', transform=ax6.transAxes, fontsize=12)
        ...
        ax8.hist(pagerank_values, bins=30, alpha=0.7, color='magenta', edgecolor='black')
    ...
    def generate_comprehensive_report(self):
        ...
        with open(report_file, 'w', encoding='utf-8') as f:
            ...
            f.write(f"Network Density: {self.analysis_results['metrics']['density']:.6f}\n")
            f.write(f"Number of Communities: {len(self.communities)}\n")
            f.write(f"Average Degree: {self.analysis_results['metrics']['avg_degree']:.2f}\n")
            f.write(f"Degree Standard Deviation: {self.analysis_results['metrics']['degree_std']:.2f}\n")
            f.write(f"Maximum Degree: {self.analysis_results['metrics']['max_degree']}\n")
            f.write(f"Minimum Degree: {self.analysis_results['metrics']['min_degree']}\n")
            f.write(f"Average Clustering Coefficient: {self.analysis_results['metrics']['avg_clustering']:.4f}\n\n")
            ...
            if self.analysis_results['metrics']['is_connected']:
                ...
                f.write(f"Network Diameter: {self.analysis_results['metrics']['diameter']}\n")
                f.write(f"Average Shortest Path Length: {self.analysis_results['metrics']['avg_shortest_path']:.2f}\n")
            else:
                f.write(f"Network Status: DISCONNECTED\n")
                f.write(f"Number of Connected Components: {self.analysis_results['metrics']['components']}\n")
                f.write(f"Largest Component Size: {self.analysis_results['metrics']['largest_component_size']:,}\n")
            f.write("\n")
            ...
            f.write("-" * 40 + "\n")
            community_sizes = [(i, len(comm)) for i, comm in enumerate(self.communities)]
            top_communities = sorted(community_sizes, key=lambda x: x[1], reverse=True)[:10]
            ...
            f.write("-" * 40 + "\n")
            f.write(f"This analysis examined a network of {self.G.number_of_nodes():,} onion sites\n")
            f.write(f"connected by {self.G.number_of_edges():,} links, revealing {len(self.communities)}\n")
            f.write(f"distinct communities. The network density is {self.analysis_results['metrics']['density']:.6f},\n")
            f.write(f"indicating a {'sparse' if self.analysis_results['metrics']['density'] < 0.01 else 'moderate' if self.analysis_results['metrics']['density'] < 0.1 else 'dense'}\n")
            f.write(f"connection structure.\n\n")
            
            if self.analysis_results['metrics']['is_connected']:
                f.write(f"The network is fully connected with a diameter of {self.analysis_results['metrics']['diameter']}\n")
                f.write(f"and average path length of {self.analysis_results['metrics']['avg_shortest_path']:.2f},\n")
                f.write(f"suggesting {'small-world' if self.analysis_results['metrics']['avg_shortest_path'] < 6 else 'large-world'} characteristics.\n")
            else:
                f.write(f"The network consists of {self.analysis_results['metrics']['components']} disconnected components,\n")
                f.write(f"with the largest component containing {self.analysis_results['metrics']['largest_component_size']:,} nodes.\n")
            
            f.write(f"\nCommunity detection revealed a modular structure with community sizes ranging\n")
            f.write(f"from {min(community_sizes)} to {max(community_sizes)} nodes, suggesting varying levels\n")
    ...
    def run_complete_analysis(self):
        ...
        try:
            ...
            print(f"Network analyzed: {self.G.number_of_nodes():,} nodes, {self.G.number_of_edges():,} edges")
