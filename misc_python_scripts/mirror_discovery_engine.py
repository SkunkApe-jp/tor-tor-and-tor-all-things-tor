...
PORTAL_HTML_FILE = "../scraped_data/mirror_discovery_portal.html"
...
def get_mirror_intelligence():
    ...
    onion_pattern = re.compile(r'^[a-zA-Z0-9_.-]{1,100}$')
    ...
    for item in base_path.iterdir():
        if item.is_dir() and onion_pattern.match(item.name):
            ...
            # 1. Search for ANY available title file (priority to root index)
            title_dir = item / "website_identity"
            possible_titles = list(title_dir.glob("*_title.txt")) if title_dir.exists() else []
            if possible_titles:
                # Prioritize root index, but fallback to sub-paths if root hasn't been scraped
                title_file = title_dir / "index_title.txt"
                if not title_file.exists():
                    title_file = possible_titles[0]
                try:
                    content = title_file.read_text(encoding='utf-8').strip()
                    match = re.search(r'\[(.*?)\]', content)
                    title = match.group(1).strip() if match else content.split('\n')[0].strip()
                except: pass
            
            # 2. Search for ANY available screenshot (priority to root index)
            images_dir = item / "images"
            possible_images = list(images_dir.glob("*.png")) if images_dir.exists() else []
            rel_img = ""
            if possible_images:
                img_path = images_dir / "index.png"
                if not img_path.exists():
                    img_path = possible_images[0]
                rel_img = f"scraped_data/{addr}/images/{img_path.name}"
    ...
    <style>
        ...
        * {{ box-sizing: border-box; }}
        ...
        .btn-del {{
            ...
            transition: 0.2s; font-size: 0.9rem; margin-left: 5px;
        }}

        .btn-del:hover {{ color: var(--delete); transform: scale(1.1); }}
...
/_/  /_/_/_/  /_/   \____/_/           /_____/_/____/\___/\____/|___/\___/_/   \__, /   
    ...
    <script>
                ...
                uniqueOnions.forEach((o, addr) => {{
                    ...
                    linksHtml += `
                        <div class="onion-node" id="node-${{addr}}">
                            ...
                            ${{statsHtml}}
                            <button class="btn-del" title="Delete Individual Target" onclick="deleteNode('${{addr}}', this)"><i class="fas fa-times"></i></button>
                        </div>
                    `;
                }});
        ...
        async function deleteNode(address, btnElement) {{
            if(!confirm(`Are you sure you want to permanently delete the scraped data for ${{address}}?`)) return;
            try {{
                const baseUrl = window.location.protocol === "file:" ? "http://localhost:8989" : "";
                const response = await fetch(`${{baseUrl}}/api/delete/${{address}}`, {{ method: 'POST' }});
                if (response.ok) {{
                    const nodeEl = document.getElementById(`node-${{address}}`);
                    if (nodeEl) nodeEl.remove();
                }} else {{
                    alert("Delete failed. Make sure the API server is running.");
                }}
            }} catch (e) {{
                alert("Delete blocked! Ensure the Python Engine is running on port 8989.");
            }}
        }}
