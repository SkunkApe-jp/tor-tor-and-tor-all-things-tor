...
<body>
    ...
    <script>
        ...
        // Create groups for each image node to hold both the image and its border
        const imageGroups = g.selectAll(".image-group")
            .data(imageData)
            .enter()
            .append("g")
            .attr("class", "image-group");

        // Add rectangular border for each image
        imageGroups.append("rect")
            .attr("x", d => d.x - d.width/2 - 2)  // Position with slight offset for border width
            .attr("y", d => d.y - d.height/2 - 2)  // Position with slight offset for border width
            .attr("width", d => d.width + 4)  // Width slightly larger than the image
            .attr("height", d => d.height + 4)  // Height slightly larger than the image
            .attr("fill", "none")
            .attr("stroke", "#000")  // Black border
            .attr("stroke-width", 1)  // Fine border
            .attr("rx", 5)  // Rounded corners
            .attr("ry", 5);  // Rounded corners

        // Add images to the visualization
        const imageNodes = imageGroups
            .append("image")
            ...
            .on("click", function(event, d) {{
                ...
                // Look for the onion address in the path (typically 56-character base32 string)
                for (let i = 0; i < pathParts.length; i++) {{
                    if (pathParts[i].length === 56 && /^[a-z2-7]+$/.test(pathParts[i])) {{
...
def main():
    ...
    parser.add_argument('--output', default='../scraped_data/umap_index_clusters.html',
                        help='Output HTML file path (default: ../scraped_data/umap_index_clusters.html)')
