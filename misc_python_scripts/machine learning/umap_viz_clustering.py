#!/usr/bin/env python3
"""
HTML Visualization Clustering with UMAP

Creates a visualization that groups HTML visualization files from onion sites using UMAP dimensionality reduction.
"""
...
def find_visualization_images(scraped_data_dir):
    """
    Find all images in the images subdirectory of each onion directory.
    """
    image_paths = []
    ...
    for item in os.listdir(scraped_data_dir):
        ...
        # Check if it's a directory that looks like an onion address
...
def generate_d3_visualization(coords, image_paths, output_file):
    """
    Generate HTML with D3 visualization for UMAP clustered visualization images.
    """
...
<html>
<head>
    <title>UMAP Visualization Image Clustering</title>
...
<body>
    <div id="title">UMAP Visualization Image Clustering</div>
    <div id="controls">
        <button class="control-btn" onclick="zoomIn()">+</button>
        <button class="control-btn" onclick="zoomOut()">-</button>
        <button class="control-btn" onclick="resetView()">↺</button>
    </div>
    <div id="info">Displaying {{len(image_data)}} visualization images clustered by visual similarity</div>
    <div id="tooltip"></div>
    <script>
        // Image data
        ...
        const imageNodes = g.selectAll(".image-node")
            ...
            .on("mouseover", function(event, d) {{
                tooltip.transition()
                    .duration(200)
                    .style("opacity", .95);
                tooltip.html(`<div><strong>${{d.filename}}</strong></div><div>Onion: ${{d.onion_address}}</div><img src="${{d.src}}" style="max-width: 200px; max-height: 150px; margin-top: 5px; border: 1px solid #ddd;" onerror="this.parentNode.innerHTML+='<div style=\\'color:red; font-size:12px;\\'>Image not available</div>';">`)
            ...
            .on("click", function(event, d) {{
                // Extract the onion address from the data
                if (d.onion_address) {{
...
def main():
    parser = argparse.ArgumentParser(description='Create UMAP clustering visualization for onion site visualization images')
    ...
    parser.add_argument('--output', default='../scraped_data/umap_viz_clusters.html',
                        help='Output HTML file path (default: ../scraped_data/umap_viz_clusters.html)')
    ...
    # Find all images in the images subdirectories of onion directories
    print(f"Searching for images in {args.input_dir}...")
    image_paths = find_visualization_images(args.input_dir)
